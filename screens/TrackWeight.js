...
const TrackWeight = () => {
  ...
  const loadRecords = useCallback(async () => {
    ...
    try {
      let q;
      try {
        q = query(collection(db, "users", user.uid, "trackingweight"), orderBy("createdAt", "desc"));
  ...
  const save = useCallback(async () => {
    ...
    try {
      ...
      await addDoc(collection(db, "users", user.uid, "trackingweight"), recordData);
      setWeight("");
      setShowList(true);
      setSaveSuccess(true);
      await loadRecords();
  ...
  }, [date, weight, user, loadRecords]);

  const remove = async (id) => {
    if (!user) return;
    try {
      await deleteDoc(doc(db, "users", user.uid, "trackingweight", id));
      await loadRecords();
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      <Text variant="headlineSmall" style={{ marginBottom: 8 }}>{t("weight")}</Text>
      ...
      <TextInput label={t("weight_kg")} value={weight} onChangeText={setWeight} keyboardType="numeric" style={styles.input} mode="outlined" />
          ...
          {records.length === 0 ? (
            ...
            records.map((r) => (
              <Card key={r.id} style={{ marginBottom: 10, borderRadius: 12 }}>
                <Card.Title title={`${new Date(r.date).toDateString()}`} subtitle={`${t("weight_kg")}: ${r.weight}kg`} right={(props) => (
